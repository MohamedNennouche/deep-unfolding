...
def test_solution_shape(generate_data):
    ...
    assert solution.shape == (
        10,
        300,
    ), "The solution tensor must be of the form (bs, n)"
...
def test_inverse_diagonal(decompose_data):
    ...
    assert torch.allclose(
        D @ Dinv, identity, atol=1e-6
    ), "Dinv should be the inverse of D"
...
def test_inverse_D_plus_L(decompose_data):
    ...
    identity = torch.eye(D.shape[0]).to(D.device)
    assert torch.allclose(
        (D + L) @ Minv, identity, atol=1e-6
    ), "Minv should be the inverse of D + L"
